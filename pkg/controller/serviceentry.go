...
func (r *serviceEntryController) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	serviceEntryLog.Infof("reconcile: %s/%s", request.Namespace, request.Name)

	// Fetch the ReplicaSet from the cache
	se := &networking.ServiceEntry{}
	err := r.Get(ctx, request.NamespacedName, se)
	...
	r.autoAllocateIP(request.NamespacedName, se)
	return reconcile.Result{}, nil
}
